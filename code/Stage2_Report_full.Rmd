...
different orientations[\^In the pre-registered plan, we used the term
"imagery score" but this term was confusing. Therefore, we used "mental
rotation scores" instead of "imagery scores" in the final report.].
Their analysis showed that mental rotation affected the Dutch
participants' sentence-picture verification performance. With the
measurement of mental rotation scores, we explore the association of
spatial cognition and the effect of orientation in comprehension across
...
or 0.10)[\^ See details of power analysis in the pre-registered plan,
pp. 13 - 15. <https://psyarxiv.com/t2pjv/>].

We collected data in `r length(unique(SP_V$Language))` languages from
`r length(unique(c(SP_V$PSA_ID, PP$PSA_ID)))` laboratories. Each
laboratory chose a maximal sample size and an incremental *n* for
...
`r format(length(unique(c(SP_V$Subject[SP_V$opensesame_codename == "osweb"], PP$Subject[PP$opensesame_codename == "osweb"]))), scientific = FALSE, big.mark = ",")`
completing the online version[^1]. The in-person version included
...
(TUR_007, TWN_002), two sets of data were excluded from all analyses due
...
(<https://codeocean.com/capsule/9287673>). Appendix B summarizes the
average characteristics by language and laboratory.

[^1]: Data for this study was collected together with another unrelated
...
included a potential object for depiction. For example, *After a week
the painting arrived by mail*, and *The flowers that were planted last
...
sentences for implied orientation. *The librarian put the book back on
the table* was matched with a horizontally-oriented book, while *The
...
<https://osf.io/utqxb>). Another 48 pictures were included for the
...
Then, the object picture [from @zwaanRevisitingMentalSimulation2012] was
...
[@stanfield_effect_2001], a memory check test was carried out after
every three to eight trials to ensure that participants had read each
sentence carefully.
...
To test hypothesis 1, our first planned analysis[^2] used a
...
[^2]: See the analysis plan in the pre-registered plan, pp. 19 - 20,
...
coefficients and meta-analysis [@lukeEvaluatingSignificanceLinear2017].
*p*-values were interpreted using the pre-registered $\alpha$ level of
.05.
...
Whereas the AIC values indicated a significant change, the *p*-value did
...
[@chenDoesObjectSize2020; @stanfield_effect_2001;
@zwaanRevisitingMentalSimulation2012]. The only language in which we
...
[@kaschakEmbodimentLabTheory2021]. Below, we summarize the lessons and
...
@chenDoesObjectSize2020 used the probe sentences in which the target
objects were the subject of sentences (e.g., *The **nail** was hammered
...
used in @stanfield_effect_2001, in which the target objects are the
object of sentences (e.g., *The carpenter hammered the **nail** into the
...
statistical community regarding the precision of the *p* values computed
for linear mixed models [@bolker2015]. One alternative,
...
[@matuschek2017]. Using this approach in an exploratory analysis, we
...
@sala2017]. However, research on embodied cognition should continue with
...
[@cohen1983]. Furthermore, in addition to categorical versus continuous
...
[@barsalouEstablishingGeneralizableMechanisms2019; @noah2018]. Last, and
...
1,000 participants [@bernabeu2022]. Examining these and other sources of
...
words) is not dependent on modality-specific systems (e.g., vision,
hearing, touch, interoception, motor control, emotion, etc.). So far,
some of these comparisons have supported both accounts. However, in some
studies, the effects of the symbolic account have been larger than those
of the embodied account [@bernabeu2022; @louwerse2015], whereas the
reverse has been true in other studies [@fernandino2022; @tong2022].

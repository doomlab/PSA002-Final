...
author:
  ...
  - name: Brianna Jurosic
    ...
    role:
      - Investigation
      - Writing - review & editing
    email: bjurosic@ashland.edu
  - name: Brynna Leach
    affiliation: '24'
    role:
      - Investigation
      - Writing - review & editing
    email: bleach@ashland.edu
  - name: Savannah Lewis
    affiliation: '24'
    role:
      - Investigation
      - Writing - review & editing
    email: slewis16@ashland.edu
  - name: Peter R. Mallik
    affiliation: '25'
    role:
      - Investigation
      - Writing - review & editing
    email: pmallik@ashland.edu
...
affiliation:
  ...
  - id: '20'
    institution: Department of Psychiatry and Psychological Health, Universiti Malaysia
  ...
  - id: '26'
    institution: School of Psychological and Behavioral Sciences, Southern Illinois
  ...
  - id: '34'
    institution: Department of Cognition, Emotion, and Methods in Psychology, Faculty
      of Psychology, University of Vienna, Wien, Austria
  - id: '35'
  ...
  - id: '38'
    institution: School of Studies in Life Science, Pt. Ravishankar Shukla University,
      Raipur, India
  - id: '39'
    institution: Department of Biological and Medical Psychology, University of Bergen,
      Bergen, Norway
  - id: '40'
  ...
  - id: '57'
    institution: Department of Psychology, Middle East Technical University, Ankara,
      Turkey
  - id: '58'
    institution: Department of Psychological Science, Central Connecticut State University,
      New Britain, CT, USA
  - id: '59'
...
@zwaan_embodiment_2014]. Proponents of situated cognition assume that
perceptual representations are able to be generated during language
processing [@barsalouPerceptualSymbolSystems1999; @wilson2002].
...
patterns from seeing visual images and reading text [see the summary of
@ostarekSixChallengesEmbodiment2019].
...
and orientation effects [@koning_mental_2017;
@rommersObjectShapeOrientation2013;
@zwaanRevisitingMentalSimulation2012], and the theoretical frameworks do
...
[@zwaanEmbodiedSentenceComprehension2005;
...
and Chinese [@chenDoesObjectSize2020], null evidence in Dutch
[@rommersObjectShapeOrientation2013; @koning_mental_2017], and German as
second language [@kosterMentalSimulationObject2018]. Among the studies
...
in @zwaanRevisitingMentalSimulation2012; 0.07 vs. 0.27 in
...
[@newmanCrosslinguisticOverviewPosture2002;
@verkerkEvolutionaryDynamicsMotion2014]. In addition, the potential role
of mental rotation as a confound has been considered
[@rommersObjectShapeOrientation2013]. We expand on how the context,
experimental, and cognitive factors hinder the improvement of
theoretical frameworks below.

**Context Factors.** The probe sentences used in object orientation
...
@verkerkEvolutionaryDynamicsMotion2014, Germanic languages (e.g., Dutch,
...
@verkerkEvolutionaryDynamicsMotion2014 notes that path-only
...
verbs indicating existence. As @newmanCrosslinguisticOverviewPosture2002
...
and situated simulation, @barsalou_grounded_2008] do not precisely
...
required participants to verify the probe sentences they had read [see
@zwaan_embodiment_2014]. Without such a verification, participants might
...
objects [e.g., @zwaanReadersConstructSpatial1993]. In this regard, it is
...
influence the results [@barsalouEstablishingGeneralizableMechanisms2019;
...
could increase with the depicted object size. @chenDoesObjectSize2020
...
[@cohenMentalRotationMental1993]. In each trial of this task, two
...
@chenDoesObjectSize2020 depicted that mental rotation may affect the
...
different orientation[^1]. With this measurement, we explore the
...
[^1]: In the preregistered plan, we used the term "imagery score" but
...
original methods from @stanfield_effect_2001 and addressed two primary
...
(2) Based on the assumption that the mental rotation is a general
    cognitive function, we expect equal mental rotation scores across
    languages but no association with mental simulation effects [see
...
The preregistered power analysis indicated *n* = 156 to 620 participants
for 80% power for a directional one-sample *t*-test for a *d* = 0.20 and
0.10, respectively. A mixed-model simulation suggested that *n* = 400
...
as @zwaanRevisitingMentalSimulation2012. The laboratories were allowed
...
$BF_{10}$ = 10 or $\frac{1}{10}$)[^2].

[^2]: See details of power analysis in the preregistered plan, p. 13 \~
    15. <https://psyarxiv.com/t2pjv/>

We collected data in `r length(unique(SP_V$Language))` languages
from `r length(unique(c(SP_V$PSA_ID, PP$PSA_ID)))` laboratories. Each
...
the source files (<https://codeocean.com/capsule/9287673>). Appendix B
...
that they understood the sentence. Then, the object picture [from
@zwaanRevisitingMentalSimulation2012] was presented in the center of the
...
Following the original study [@stanfield_effect_2001], a memory check
...
([https://psysciacc.org/translation-process/](https://www.google.com/url?q=https://psysciacc.org/translation-process/&sa=D&source=docs&ust=1680671843449808&usg=AOvVaw2Lp4-G01CyfxaMyrZfwvmy)).
In addition, the coordinator and participating laboratories consulted
about each of the following points:

1) The translators could denote the items that are unfamiliar for a
...
For example, American individuals use **tire**; British individuals use
**tyre**. Every coordinator would mark these local translations in the
...
Our first planned analysis[^4] employed a random-effects meta-analysis
...
[^4]: See the analysis plan in the preregistered plan, p. 19 \~ 20.
...
factors (such as the participants\' language proficiency or backgrounds)
...
[@stanfield_effect_2001; @zwaanRevisitingMentalSimulation2012, the
...
team-based effects (USA_173, USA_030 and USA_032, see Figure 3), and
also three significant negative effects (USA_33, USA_20, and GBR_005,
see Figure 3).
...
this project. @chenDoesObjectSize2020 used the probe sentences in which
the target objects were the subject of sentences (e.g., The **nail** was
...
trials [@chenDoesObjectSize2020; @kaschakEmbodimentLabTheory2021;
@stanfield_effect_2001] showed a positive object orientation effect. In
...
participants had a higher percent correct than in-person participants, *t*(`r apa_num(t_test$parameter)`) = `r apa_num(t_test$statistic)`, *p* `r apa_p(t_test$p.value)`, $M_{online}$ = `r apa_num(mean(SP_M_out$M_acc[SP_M_out$Source == "osweb"], na.rm = T)*100)` (*SD* = `r apa_num(sd(SP_M_out$M_acc[SP_M_out$Source == "osweb"], na.rm = T)*100)`) and $M_{in-person}$ = `r apa_num(mean(SP_M_out$M_acc[SP_M_out$Source == "site"], na.rm = T)*100)` (*SD* = `r apa_num(sd(SP_M_out$M_acc[SP_M_out$Source == "site"], na.rm = T)*100)`). 
...
participants\' language backgrounds and the difference of stimuli
